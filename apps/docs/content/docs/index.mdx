...
## Gating updates with `deps`

By default the bridge runs `capture`/`apply` on **every** update, even ones unrelated to the DOM you measure (an unrelated `useState`, a hover toggle, etc.). Since `capture` often forces a synchronous layout read (`scrollHeight`, `getBoundingClientRect`), running it on irrelevant commits is wasted work.

Pass a `deps` array — compared shallowly (`Object.is`) against the previous commit, just like `useEffect` — to run both callbacks only on commits where a dependency changed.
...
function ChatList({ messages }: { messages: string[] }) {
  ...
  return (
    <SnapshotBeforeUpdate<number>
      // Only runs on commits where the message count changed,
...
Prefer primitive values (`messages.length`, an `id`) in `deps`. Inline objects/arrays (`deps={[{ ... }]}`) get a new reference every render, so they never compare equal and the gate has no effect — the same pitfall as `useEffect`.
